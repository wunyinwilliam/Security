...
class AddWords:
    ...
    def create_description_text(self, image):
        ...
        x, y, w, h = width-300, 0, 300, 130
        sub_img = image[y:y+h, x:x+w]
        white_rect = np.ones(sub_img.shape, dtype=np.uint8) * 255
        res = cv2.addWeighted(sub_img, 0.5, white_rect, 0.5, 1.0)
        cv2.putText(res, 'Double Click: Start Typing/Delete', (0, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        cv2.putText(res, 'Enter: Finish Typing & Save', (0, 40), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        cv2.putText(res, 'Clear All: F1', (0, 60), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        cv2.putText(res, 'Normal/Delete Mode: F2', (0, 80), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        cv2.putText(res, 'White/Black Text: F3', (0, 100), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        cv2.putText(res, 'Exit: Esc', (0, 120), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 0), 1)
        ...
        if event == cv2.EVENT_LBUTTONDBLCLK:
            if not self.is_delete:
                ...
                if self.is_white:
                    ...
                    cv2.rectangle(self.word_img_white, (self.text_location[0]+self.text_pointer_location_x, self.text_location[1]-30), (self.text_location[0]+self.text_pointer_location_x+1, self.text_location[1]+10), (255, 255, 255), -1)
                else:
                    self.image_without_new_words = self.word_img_black.copy()
                    self.word_img_without_pointer = self.word_img_black.copy()
                    cv2.rectangle(self.word_img_black, (self.text_location[0]+self.text_pointer_location_x, self.text_location[1]-30), (self.text_location[0]+self.text_pointer_location_x+1, self.text_location[1]+10), (255, 255, 255), -1)
                ...
                for word_history in self.word_history:
                    if word_history[3]:
                        cv2.putText(self.word_img_white, word_history[0], word_history[1], cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
                    else:
                        cv2.putText(self.word_img_black, word_history[0], word_history[1], cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
                self.save_word_img()
            ...
            if self.word_img_without_pointer is not None:
                if self.is_white:
                    ...
                    if self.text_input != '' and self.text_location is not None and self.text_pointer_location_x != 0:
                        self.word_history.append((self.text_input, self.text_location, self.text_pointer_location_x, True))
                else:
                    self.word_img_black = self.word_img_without_pointer.copy()
                    if self.text_input != '' and self.text_location is not None and self.text_pointer_location_x != 0:
                        self.word_history.append((self.text_input, self.text_location, self.text_pointer_location_x, False))
            self.save_word_img()
        else:
            if self.is_white:
    ...
    def start(self):
        while True:
            ...
            if (key > 31) and (key < 127):        # letters or integers
                if self.is_typing:
                    if self.is_white:
                        self.word_img_white = self.image_without_new_words.copy()
                        self.text_input += chr(key%256) if key%256 < 128 else '?' 
                        cv2.putText(self.word_img_white, self.text_input, self.text_location, cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
                        self.word_img_without_pointer = self.word_img_white.copy()
                        self.text_pointer_location_x += 20 
                        cv2.rectangle(self.word_img_white, (self.text_location[0]+self.text_pointer_location_x, self.text_location[1]-30), (self.text_location[0]+self.text_pointer_location_x+1, self.text_location[1]+10), (255, 255, 255), -1)
                    else:
                        self.word_img_black = self.image_without_new_words.copy()
                        self.text_input += chr(key%256) if key%256 < 128 else '?' 
                        cv2.putText(self.word_img_black, self.text_input, self.text_location, cv2.FONT_HERSHEY_PLAIN, 2, (255, 255, 255), 2)
                        self.word_img_without_pointer = self.word_img_black.copy()
                        self.text_pointer_location_x += 20 
                        cv2.rectangle(self.word_img_black, (self.text_location[0]+self.text_pointer_location_x, self.text_location[1]-30), (self.text_location[0]+self.text_pointer_location_x+1, self.text_location[1]+10), (255, 255, 255), -1)
